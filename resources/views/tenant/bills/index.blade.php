...
<<<<<<< HEAD
    ...
    <div class="bg-white rounded-2xl border overflow-hidden">
        <div class="px-6 py-5 border-b">
            <h2 class="text-xl font-black text-slate-800">Hóa đơn của bạn</h2>
            <p class="text-sm text-slate-500">Xem / tải PDF và gửi chứng từ thanh toán</p>
        </div>
                ...
                <tbody>
                    ...
                    <tr class="border-t">
                        ...
                        <td class="px-6 py-4 text-center">
                            @if($i->status === 'paid')
                                <span class="px-3 py-1 rounded-full bg-emerald-50 text-emerald-700 text-xs font-black">ĐÃ THANH TOÁN</span>
                            @else
                                <span class="px-3 py-1 rounded-full bg-rose-50 text-rose-600 text-xs font-black">CHƯA THANH TOÁN</span>
                            @endif
                        </td>
                        ...
                        <td class="px-6 py-4 text-center">
                            ...
                            @else
                                <span class="text-slate-400 text-sm">Chưa có PDF</span>
                            @endif
                        </td>
                        ...
                        <td class="px-6 py-4 text-right">
                            ...
                            @else
                                ...
                                <form method="POST"
                                    ...
                                    <input type="text" name="note" placeholder="Ghi chú..." class="px-3 py-2 rounded-lg border">

                                    <button type="submit" class="px-4 py-2 rounded-lg bg-slate-900 text-white font-bold">
                                        Gửi
                                    </button>
                                </form>
                            @endif
...
=======
    ...
    <div class="bg-white rounded-[2.5rem] border border-slate-100 shadow-xl shadow-slate-200/50 overflow-hidden mb-12">
        <div class="px-8 py-6 bg-slate-50/50 border-b border-slate-100 flex items-center justify-between">
            ...
            <span class="px-3 py-1 bg-white border border-slate-200 rounded-lg text-[10px] font-bold text-slate-500">Tháng {{ date('m/Y') }}</span>
        </div>
                        ...
                        <tr class="group hover:bg-slate-50/80 transition-all duration-300">
                            <td class="px-8 py-6">
                                <div class="flex items-center gap-4">
                                    ...
                                    <div>
                                        <p class="font-black text-slate-800 tracking-tight">{{ $i->contract?->room?->room_code ?? '---' }}</p>
                                        <p class="text-[11px] font-bold text-slate-400">Kỳ: {{ str_pad($i->month, 2, '0', STR_PAD_LEFT) }}/{{ $i->year }}</p>
                                    </div>
                                </div>
                            </td>

                            <td class="px-8 py-6 text-center">
                                <span class="text-lg font-black text-slate-900 tracking-tighter">{{ number_format($i->total ?? 0) }}<span class="text-xs ml-1 text-slate-400 font-medium">đ</span></span>
                            </td>
                            ...
                            <td class="px-8 py-6 text-center">
                                ...
                                @else
                                    <span class="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-rose-100 text-rose-700 text-[10px] font-black uppercase tracking-wide border border-rose-200 animate-pulse">
                                        <span class="w-1.5 h-1.5 rounded-full bg-rose-500"></span> Chưa thanh toán
                                    </span>
                                @endif
                            </td>
                            ...
                            <td class="px-8 py-6 text-center">
                                @if($i->invoice_file)
                                    <div class="flex items-center justify-center gap-2">
                                        <a href="{{ route('tenant.bills.view', $i->id) }}" target="_blank" class="w-8 h-8 flex items-center justify-center rounded-lg bg-white border border-slate-200 text-slate-600 hover:bg-indigo-600 hover:text-white hover:border-indigo-600 transition-all shadow-sm" title="Xem PDF">
                                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"/><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"/></svg>
                                        </a>
                                        <a href="{{ route('tenant.bills.download', $i->id) }}" class="w-8 h-8 flex items-center justify-center rounded-lg bg-white border border-slate-200 text-slate-600 hover:bg-emerald-600 hover:text-white hover:border-emerald-600 transition-all shadow-sm" title="Tải về">
                            ...
                            <td class="px-8 py-6 text-right">
                                @if($i->status === 'paid')
                                    <span class="text-xs font-black text-emerald-600 bg-emerald-50 px-4 py-2 rounded-xl italic">Giao dịch hoàn tất</span>
                                @elseif($payment && $payment->status === 'pending')
                                    <div class="inline-flex flex-col items-end gap-1">
                                        <span class="px-4 py-2 rounded-xl bg-amber-50 text-amber-700 font-black text-[10px] uppercase tracking-wider border border-amber-100">⏳ Chờ duyệt</span>
                                        <button type="button" @click="openProof = true; proofImg = '{{ asset('storage/'.$payment->image_path) }}'" class="text-[10px] font-bold text-indigo-500 hover:underline tracking-tight">Xem chứng từ đã gửi</button>
                                    </div>
                                @else
                                    <form method="POST" action="{{ route('tenant.bills.payment.store', $i->id) }}" enctype="multipart/form-data" class="flex flex-col md:flex-row items-center gap-2 justify-end group/form">
                                        @csrf
                                        ...
                                        <input type="text" name="note" placeholder="Ghi chú..." class="w-32 px-3 py-2 text-[11px] font-bold rounded-xl border border-slate-200 focus:ring-4 focus:ring-indigo-500/10 focus:border-indigo-500 outline-none transition-all">
                                        <button type="submit" class="px-5 py-2 bg-slate-900 text-white rounded-xl text-[10px] font-black uppercase tracking-widest hover:bg-indigo-600 transition-all shadow-lg active:scale-95">
                                            {{ ($payment && $payment->status === 'rejected') ? 'Gửi lại' : 'Gửi' }}
                                        </button>
                                    </form>
                                    @if($payment && $payment->status === 'rejected')
                                        <p class="text-[10px] text-rose-500 font-bold mt-1 italic">Lý do từ chối: {{ $payment->note }}</p>
                                    @endif
                                @endif
                            </td>
                        </tr>
...
=======
            ...
            @forelse($approvedInvoices as $i)
                ...
                <div class="bg-white p-6 rounded-[2rem] border border-slate-100 shadow-sm hover:shadow-xl hover:shadow-slate-200/50 transition-all duration-300 group">
                    ...
                    <div class="space-y-1 mb-6">
                        <p class="text-sm font-bold text-slate-500 uppercase tracking-tighter leading-none">Phòng {{ $i->contract?->room?->room_code }}</p>
                        <p class="text-2xl font-black text-slate-900 tracking-tighter leading-none">{{ number_format($i->total ?? 0) }} đ</p>
                    </div>

                    <div class="flex items-center justify-between pt-4 border-t border-slate-50">
                        @if($payment && $payment->image_path)
                            <button @click="openProof = true; proofImg = '{{ asset('storage/'.$payment->image_path) }}'" class="flex items-center gap-2 text-[10px] font-black text-indigo-500 uppercase tracking-widest hover:text-indigo-700 transition-colors">
    ...
    {{-- MODAL XEM CHỨNG TỪ (FANCY VERSION) --}}
    <div x-show="openProof" x-cloak
        ...
        <div class="relative z-[110] bg-white rounded-[3rem] p-4 shadow-2xl max-w-2xl w-full">
            <button @click="openProof = false" class="absolute -top-12 right-0 text-white flex items-center gap-2 font-black uppercase text-[10px] tracking-widest">
                Đóng (Esc) <svg class="w-8 h-8 rounded-full bg-white/10 p-2" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"/></svg>
            </button>
