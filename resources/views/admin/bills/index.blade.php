            ...
            <table class="w-full">
                ...
                <tbody class="divide-y">
                    @forelse($invoices as $i)
                        @php
                            $roomCode = $i->contract?->room?->room_code ?? 'N/A';
                        ...
                        <tr class="hover:bg-slate-50 transition-colors">
                            <td class="px-6 py-4">
                                <div class="font-black text-slate-700">Phòng: {{ $roomCode }}</div>
                            </td>
                            ...
                            <td class="px-6 py-4 text-center">
                                <span class="text-sm font-black text-slate-800">{{ $fmtVnd($i->total) }}</span>
                            </td>
                            ...
                            <td class="px-6 py-4 text-right">
                                <div class="flex justify-end gap-2 items-center">
                                    ...
                                    @else
                                        <span class="px-3 py-2 rounded-lg bg-slate-100 text-slate-400 font-bold text-sm cursor-not-allowed">🔒 Đã thanh toán</span>
                                    @endif
...
<div id="modal" class="fixed inset-0 bg-black/40 hidden items-center justify-center z-[300] p-4">
    <div class="bg-white w-full max-w-2xl rounded-xl animate-modal overflow-hidden max-h-[95vh] flex flex-col shadow-2xl">
        <div class="p-6 border-b flex items-center justify-between bg-white sticky top-0 z-10">
            ...
            <button type="button" onclick="BillsUI.closeModal()" class="text-slate-400 hover:text-rose-500 font-black text-xl transition-colors">✕</button>
        </div>

        <div class="p-6 overflow-y-auto modal-scroll">
            <form method="POST" action="{{ route('admin.bills.store') }}" class="space-y-5" id="billForm">
                @csrf
                    ...
                    <select name="room_id" id="roomSelect" required
                        ...
                        @foreach($rooms as $r)
                            <option value="{{ $r->id }}"
                                ...
                                {{ $r->room_code }} ({{ $r->active_contract_id ? 'Có HĐ active' : 'Chưa có HĐ active' }})
                            </option>
                        @endforeach
                    </select>
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                        ...
                        <input type="number" name="month" min="1" max="12" required placeholder="2" value="{{ old('month') }}"
                               class="w-full p-3 bg-slate-50 rounded-xl font-bold text-slate-700 border border-transparent focus:border-indigo-500 outline-none">
                    </div>
                    <div>
                        <label class="text-xs font-bold text-slate-500 mb-1 block">Năm</label>
                        <input type="number" name="year" min="2000" required placeholder="2026" value="{{ old('year') }}"
                               class="w-full p-3 bg-slate-50 rounded-xl font-bold text-slate-700 border border-transparent focus:border-indigo-500 outline-none">
                    </div>
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div class="space-y-3">
                        ...
                        <div class="grid grid-cols-2 gap-3">
                            <input type="number" name="electric_old" id="electricOld" min="0" placeholder="Cũ" value="{{ old('electric_old') }}"
                                   class="w-full p-3 bg-slate-50 rounded-xl font-bold text-slate-700 border border-transparent focus:border-indigo-500 outline-none">
                            <input type="number" name="electric_new" id="electricNew" min="0" placeholder="Mới" value="{{ old('electric_new') }}"
                                   class="w-full p-3 bg-slate-50 rounded-xl font-bold text-slate-700 border border-transparent focus:border-indigo-500 outline-none">
                        </div>
                        <p class="text-xs font-bold text-slate-500">Số kWh: <span id="electricUsage">0</span> — Tiền điện: <span class="text-indigo-600" id="electricCostText">0</span> đ</p>
                    </div>

                    <div class="space-y-3">
                        <label class="text-xs font-bold text-slate-500 mb-1 block">Nước</label>
                        <div class="grid grid-cols-2 gap-3">
                            <input type="number" name="water_old" id="waterOld" min="0" placeholder="Cũ" value="{{ old('water_old') }}"
                                   class="w-full p-3 bg-slate-50 rounded-xl font-bold text-slate-700 border border-transparent focus:border-indigo-500 outline-none">
                            <input type="number" name="water_new" id="waterNew" min="0" placeholder="Mới" value="{{ old('water_new') }}"
                                   class="w-full p-3 bg-slate-50 rounded-xl font-bold text-slate-700 border border-transparent focus:border-indigo-500 outline-none">
                        </div>
                        <p class="text-xs font-bold text-slate-500">Số m³: <span id="waterUsage">0</span> — Tiền nước: <span class="text-indigo-600" id="waterCostText">0</span> đ</p>
                    </div>
                </div>

                <div>
                    <div class="flex items-center justify-between mb-3">
                        <label class="text-xs font-bold text-slate-500 block">Dịch vụ phát sinh</label>
                        <button type="button" onclick="BillsCalc.addServiceRow()"
                    ...
                    <div class="mt-3 text-sm font-bold text-slate-600">Tổng dịch vụ: <span class="text-indigo-600" id="serviceSumText">0</span> đ</div>
                </div>

                <div class="pt-4 flex gap-3">
                    <button type="submit"
                            class="flex-1 py-4 bg-slate-900 text-white rounded-xl font-black uppercase tracking-widest text-xs hover:bg-indigo-600 transition-all shadow-xl shadow-indigo-100">
                        Lưu & Xuất PDF
                    </button>
                    <button type="button" onclick="BillsUI.closeModal()" class="px-8 py-4 text-slate-400 font-bold hover:text-slate-600 transition-colors">
                        Đóng
                    </button>
                </div>
            </form>
...
<script>
    ...
    const BillsCalc = (() => {
        ...
        const recalc = () => {
            ...
            let serviceSum = 0;
            document.querySelectorAll('.service-price').forEach((ip) => {
                serviceSum += toInt(ip.value);
            });
        ...
        const addServiceRow = (name = '', price = '') => {
            ...
            row.innerHTML = `
                ...
                <div class="col-span-1 flex items-center justify-end">
                    <button type="button" class="px-3 py-2 rounded-lg bg-rose-50 text-rose-600 font-bold hover:bg-rose-100 transition">✕</button>
                </div>
            `;
