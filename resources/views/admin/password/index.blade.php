    ...
    <div class="bg-white rounded-[2.5rem] shadow-[0_20px_50px_rgba(0,0,0,0.05)] border border-slate-100 overflow-hidden flex flex-col md:flex-row">
        ...
        <div class="md:w-5/12 bg-slate-900 p-10 text-white flex flex-col justify-between relative overflow-hidden">
            ...
            <div class="relative z-10">
                ...
                <h2 class="text-3xl font-black leading-tight tracking-tight">Nâng cấp <br>bảo mật.</h2>
                <p class="mt-4 text-slate-400 text-sm font-medium leading-relaxed">
                    Sử dụng mật khẩu mạnh giúp tài khoản quản trị của bạn an toàn hơn trước các truy cập trái phép.
                </p>
            </div>
        ...
        <div class="md:w-7/12 p-8 md:p-12 bg-white">
            ...
            @if($errors->any() && !$errors->has('current_password') && !$errors->has('new_password'))
                <div class="mb-6 p-4 rounded-2xl bg-rose-50 border border-rose-100 text-rose-700 text-sm font-bold flex items-center gap-2">
                    ❌ {{ $errors->first() }}
                </div>
            @endif
            ...
            <form method="POST" action="{{ route('admin.password.update') }}" class="space-y-6">
                ...
                <div class="pt-4">
                    <button type="submit" 
                        class="w-full bg-slate-900 text-white py-4 rounded-2xl font-black text-xs uppercase tracking-[0.2em] shadow-xl shadow-slate-100 hover:bg-indigo-600 transition-all hover:-translate-y-1 active:scale-95">
                        Xác nhận thay đổi
                    </button>
                </div>
            </form>
